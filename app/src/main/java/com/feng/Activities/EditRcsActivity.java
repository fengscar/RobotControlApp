...
public class EditRcsActivity extends BaseActivity implements I_Parameters,ArmProtocol{
	...
	private EditText etIpAddress,etIpPort;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		cancelToMenu=(Button)findViewById(R.id.btnUniformTitleLeft);
		cancelToMenu.setOnClickListener(btnLis);
		...
		etIpAddress.addTextChangedListener(etLis);
	...
	private void initValue() {
		...
		sortTypeSwitch.setChecked( ((String)SP.get(context, SORT_TYPE,"NODE_TYPE")).equals("NODE_TYPE")?true:false );
		autoEnterSwitch.setChecked((boolean)SP.get(context, AUTO_ENTER_WORKSPACE,false) );

		etIpAddress.setText((String)SP.get(context,IP_ADDRESS,"") );
		//		etIpPort.setText((String)SP.get(context,IP_PORT,""));
	}
	...
	class EditTextListener implements TextWatcher{
		...
		public void afterTextChanged(Editable s) {
			...
			if( getPointNum(ipStr)==3 && ipStr.length()>ipStr.lastIndexOf(".")+3 ){
				...
				SP.put(EditRcsActivity.this, IP_PORT, Integer.parseInt( etIpPort.getText().toString() ));
	...
	protected void onStop() {
		super.onStop();
		SP.put(EditRcsActivity.this, IP_ADDRESS, etIpAddress.getText().toString());
	}
}
