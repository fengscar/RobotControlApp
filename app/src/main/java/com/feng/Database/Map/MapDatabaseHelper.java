...
public class MapDatabaseHelper extends SQLiteOpenHelper implements I_MapData {
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        String sql = "alter table Route add " + ROUTE_TYPE + " integer";
...
//        L.i(LOG, "更新版本: 从" + oldVersion + "升级到" + newVersion + ",本次更新在Workspace表中添加了2个字段:工作区的尺寸大小");
    ...
    // 获取清洗区节点 (以后可能有多个清洗区)
    public List<Node> getNodeByType(String nodeType) {
        String sql = "select * from Node where " + NODE_TYPE + "=?";
        Cursor cur = mDatabase.rawQuery(sql, new String[]{nodeType});
    ...
    public List<Node> getAllNode(int wspID) {
        Cursor cur = mDatabase.rawQuery("select * from node where " + NODE_WORKSPACE
                + "=? ", new String[]{String.valueOf(wspID)});
        return getNodeList(cur);
    }

    public List<Node> getAllNodeOrderById(int wspID) {
        Cursor cur = mDatabase.rawQuery("select * from node where " + NODE_WORKSPACE
                + "=? order by " + NODE_ID, new String[]{String.valueOf(wspID)});
        return getNodeList(cur);
    }
...
//			}
    ...
    public List<Node> getNodeByRoute(int routeID) {
        String sql = "select * from node where " + NODE_ID + " in" +
                "(select " + PATH_NODEID + " from path where " + PATH_ROUTEID + "=?) order by " + NODE_ID;
        Cursor cursor = mDatabase.rawQuery(sql, new String[]{String.valueOf(routeID)});
        return getNodeList(cursor);
    }

    /**
     * 返回该路线经过 该节点的 路径 (单行道2个,普通路线1个,未经过的返回 空的LIST( size=0))
     */
    public List<Path> routeContainsNode(int routeID, int nodeID) {
        String sql = "select * from Path where " + PATH_ENDNODEID + "=?  " +
                " and " + PATH_ROUTEID + "=?";
        Cursor cursor = mDatabase.rawQuery(sql, new String[]{String.valueOf(nodeID), String.valueOf(routeID)});
        return getPathList(cursor);
    }
    ...
    public boolean updateData(Path path) {
        String sql = "update path set " +
                ...
                PATH_ANGLE + "=? ," +
                PATH_MAXSPEED + " =? ," +
                PATH_TURNTYPE + "=? where " + PATH_ROUTEID + " =? and " + PATH_ORDERID + " =?";
        try {
            mDatabase.execSQL(sql, new Object[]{
                    path.getNodeID(), path.getEndNode(), path.getYaw(), path.getDistance(),
                    path.getAngle(), path.getMaxSpeed(),
                    path.getTurnType(),
                    path.getRouteID(), path.getOrderID(),
            });
        } catch (Exception e) {
            L.e(e.toString());
            T.show("修改路径失败");
    ...
    public Path getPath(Cursor cur) {
        ...
        path.setTurnType(cur.getInt(cur.getColumnIndex(PATH_TURNTYPE)));
        return path;
    }
...
//endregion
    ...
    public boolean addData(Route route) {
        if (route.getPreID() >= 0 && !verifyPreRoute(route)) {
            return false;
        }
        try {
            mDatabase.execSQL("insert into Route(" + ROUTE_TABLE_ROW + ") values(?,?,?,?,?,?)",
                    new Object[]{route.getId(), route.getPreID(),
                            route.getName(), route.getWorkspaceID(), route.isEnabled(), route.getType()});
        } catch (Exception e) {
            L.e(LOG, "添加路线失败");
    ...
    public boolean updateData(Route newRoute) {
        ...
        String sql = "update route set " + ROUTE_ID + "=?," + ROUTE_PREID + "=?," + ROUTE_NAME + "=? ,"
                + ROUTE_ENABLED + "=? ," + ROUTE_TYPE + "=? where " + ROUTE_ID + "=?";
        try {
            mDatabase.execSQL(sql, new Object[]{
                    String.valueOf(newRoute.getId()), String.valueOf(newRoute.getPreID()), String.valueOf(newRoute.getName()),
                    newRoute.isEnabled() ? 1 : 0, newRoute.getType(), String.valueOf(newRoute.getId())});
...
//根据查询结果生成Route,请手动将cur移动到要查询的row
    public Route getRoute(Cursor cur) {
        ...
        route.setType(cur.getInt(cur.getColumnIndex(ROUTE_TYPE)));
        return route;
    }
    ...
    public List<Route> getAllRoute(int wspID) {
        Cursor cur = mDatabase.rawQuery("select * from route where " + ROUTE_WORKSPACEID + "=? order by " + ROUTE_ID,
                new String[]{String.valueOf(wspID)});
    ...
    public List<Route> getLegalPreRoute(Route route) {
        String sql = "select * from route where " + ROUTE_WORKSPACEID + "=? and " + ROUTE_ID + "!=? and " + ROUTE_PREID + "!=?" +
                " and " + ROUTE_PREID + ">=0"; //前级路线要 >=0 ,屏蔽掉停靠线和连接线
        Cursor cursor = mDatabase.rawQuery(sql, new String[]{
...
//endregion
     ...
     * PATH_ROUTEID为自增
     */
    public Route getPassNodeRouteID(int nodeID) {
    ...
    public List<Node> getSelectableNode(Node startNode) {
        String sql = "select * from node where " + NODE_ID + "!=? and " + NODE_WORKSPACE + "=? order by " + NODE_ID;
        return getNodeList(mDatabase.rawQuery(sql,
                new String[]{String.valueOf(startNode.getId()), String.valueOf(startNode.getWorkspaceID())}));
    }
    //endregion
}
